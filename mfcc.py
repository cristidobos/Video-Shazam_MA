...
# From mfcc.py
def segment_axis_alternative(arr, nwin, over):
    start = 0
    res = []
    nwin = int(nwin)
    over = int(over)
    arr = np.array(arr).flatten()
    while start + nwin <= len(arr):
        res.append(arr[start:start + nwin].copy().tolist())
        start += over
...
def mfcc(input, nwin=256, nfft=512, fs=16000, nceps=13):
    ...
    framed = segment_axis_alternative(extract, nwin, over) * w
